...
class exportinstitutesevaluates(APIView):
    def get(self, request):
        try:
            ...
            serializer = DetailInstitutionSerializerEvaluation(colegio, many=True)
            colegio_data = serializer.data

            # Crear un archivo Excel
            wb = openpyxl.Workbook()
            ws = wb.active
            ws.title = "Reporte de Instituciones"

            # Escribir encabezados
            headers = ["Nombre", "Código Modular", "Código Local","Nivel","Area","Región", "Provincia", "Distrito", "Estado de Evaluación"]
            ws.append(headers)

            # Escribir datos
            for colegio in colegio_data:
                ws.append([
                    colegio['institution']['name'],
                    colegio['modular_code'],
                    colegio['local_code'],
                    colegio['level']['name'],
                    colegio['area']['name'],
                    colegio['institution']['ubigeo']['region'],
                    colegio['institution']['ubigeo']['province'],
                    colegio['institution']['ubigeo']['district'],
                    colegio['evaluacion']
                ])
