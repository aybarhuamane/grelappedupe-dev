...
export const DocentesImport = () => {
  const [loading, setLoading] = useState(false)
  const router = useRouter()
  const params = useSearchParams()

  const detail_institution_id = Number(params.get('detail_institution_id'))

  const methods = useForm<importStudenst.ITeacherImport>()

  const onSubmit: SubmitHandler<importStudenst.ITeacherImport> = async (
    data: importStudenst.ITeacherImport
  ) => {
    setLoading(true)
    try {
      const res = await importFunctionsApi.importTeachersData({
        file: data.file,
  ...
  return (
    <div>
      <HeaderSection
        ...
        subtitle="Suba un archivo con los datos de los docentes que desea importar."
        disableAddButton
        ...
        <form>
          <main className="flex flex-col container space-y-8 py-6">
            <section className="justify-between items-end inline-flex">
              ...
              <a
                href={`${API_URL}media/01_DOCENTES_CARGA_MASIVA.xlsx`}
                target="_blank"
                ref={downloadRef}
                download
                className="hidden"
